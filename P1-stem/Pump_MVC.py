...
class Pump_Controller():
    ...
    def ImportFromFile(self, data):
        ...
        L=data[2].split()
        self.Model.FlowUnits = L[0]#JES Missing Code
        self.Model.HeadUnits = L[1]#JES Missing Code
    ...
    def SetData(self,data):
        ...
        for L in data:
            ...
            self.Model.FlowData=np.append(self.Model.FlowData, float(Cells[0])) #JES Missing Code) #remove any spaces and convert string to a float
            self.Model.HeadData=np.append(self.Model.HeadData, float(Cells[1])) #JES Missing Code) #remove any spaces and convert string to a float
            self.Model.EffData=np.append(self.Model.EffData, float(Cells[2]))#JES Missing Code) #remove any spaces and convert string to a float
...
class Pump_View():
    ...
    def DoPlot(self, Model):
        ...
        axes = self.ax
        axes.clear()
        axes.plot(Model.FlowData, Model.HeadData, 'ro', label='Head Data')
        axes.plot(headx, heady, 'r-', label=f'Head Fit R²={headRSq:.2f}')
        axes.set_xlabel(Model.FlowUnits)
        axes.set_ylabel(Model.HeadUnits, color='r')
        axes.tick_params(axis='y', labelcolor='r')

        axes2 = axes.twinx()
        axes2.plot(Model.FlowData, Model.EffData, 'go', label='Efficiency Data')
        axes2.plot(effx, effy, 'g-', label=f'Efficiency Fit R²={effRSq:.2f}')
        axes2.set_ylabel('Efficiency', color='g')
        axes2.tick_params(axis='y', labelcolor='g')

        fig = self.ax.figure
        fig.suptitle('Pump Performance')
        fig.legend()
